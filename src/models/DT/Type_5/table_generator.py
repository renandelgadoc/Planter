...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    LPM_Table = copy.deepcopy(Exact_Table)
    for f in range(num_features):
        print('')
        print('Begine transfer: Feature table ' + str(f))
        LPM_Table['feature ' + str(f)] = Table_to_LPM(LPM_Table['feature ' + str(f)], feature_width[f])
        

    print('')
    print('Begine transfer: Tree table ')
    key_name = []
    for f in range(num_features):
    ...
    # prepare default
    LPM_Table['code to vote'] = Muti_Exact_to_LPM_Concatination(Exact_Table['code to vote'], code_width_tree_feature, key_name, action_name)
    code_table_size = len(LPM_Table['code to vote'].keys())
    ...
    Planter_config['p4 config']["default lable"] = 0
    Planter_config['p4 config']["width of feature"] = feature_width
    Planter_config['p4 config']["width of code"] = code_width_tree_feature
    Planter_config['p4 config']["total width of code"] = int(np.sum(code_width_tree_feature))
...
def test_tables(sklearn_test_y, test_X, test_y):
    ...
    for i in range(np.shape(test_X.values)[0]):
        ...
        for f in range(num_features):
            ...
            lpm_code_list[f] = action[max_index]  # Choose the action with the longest prefix match
        ...
        binary_code = ''
        for f in range(num_features):
           binary_code = binary_code + ten_to_bin(int(code_list[f]), int(Planter_config['p4 config']["width of code"][f]))
        decimal_code = int(binary_code, 2)

        LPM_table = LPM_Table['code to vote']
        keys = list(LPM_table.keys())
        mask = []
        action = []
        for count in np.sort(keys):  # For each value in LPM table, check if it matches that separation key
        ...
        max_mask = max(mask)
        max_index = mask.index(max_mask)
        switch_prediction= action[max_index]  # Choose the action with the longest prefix match
...
def resource_prediction():
    ...
    print('LPM match entries: ',np.sum(Planter_config['p4 config']["code table size"]) +np.sum(Planter_config['p4 config']["used columns"]))
