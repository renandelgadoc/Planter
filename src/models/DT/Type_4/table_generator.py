...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    for idx in Exact_Table['code to vote']:
        if int(Exact_Table['code to vote'][idx]['leaf']) != default_label:
            LPM_Table['code to vote'][code_table_size] = Exact_Table['code to vote'][idx]
            code_table_size += 1
    Exact_Table['code to vote'] = copy.deepcopy(LPM_Table['code to vote'])
...
def resource_prediction():
    ...
    print('Exact match entries: ',np.sum(Planter_config['p4 config']["code table size"]) )
