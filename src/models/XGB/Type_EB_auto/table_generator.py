...
def get_path(model, conditions, path, num, leaf_info, tree_index):
    if 'children' in model.keys():
        ...
        leaf_info['tree ' + str(tree_index)] += [round(model["leaf"]/trade_off_factor, 1)]
...
def path_to_path_to_leaf(path, num_features, table, leaf_code_list):
    ...
    for p in path:
        ...
        path_to_leaf[p]['leaf'] = leaf_code_list.index(round(path[p]['leaf']/trade_off_factor, 1))
...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    # =================== accuracy trade-offs factor in config ===================
    question = 'Value of accuracy trade-offs factor?'
    default = 1
    Planter_config = take_CLI_input(Planter_config, 'model config', 'trade-offs factor', question, default,
                                    manually_input = False, numeric=True)

    global trade_off_factor
    trade_off_factor = Planter_config['model config']['trade-offs factor']
    ...
    print('Ternary_Table is generated')
