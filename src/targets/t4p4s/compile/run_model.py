...
#
# Copyright (c) 2020-2021 Mingyuan Zang
...
def file_names(Planter_config):
    ...
    model_test_root = Planter_config['directory config']['work']+'/src/targets/t4p4s/compile/model_test/test_environment'
...
def add_make_run_model(fname, config):
    ...
    with open(fname, 'w') as command:
        ...
        command.write("systemctl stop  t4p4s.service\n")
        command.write("cd /root/t4p4s\n")
        command.write("./t4p4s.sh :myprog p4rt verbose dbg\n")
