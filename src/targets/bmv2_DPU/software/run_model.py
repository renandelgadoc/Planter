...
def file_names(Planter_config):
    ...
    test_file_name = 'test_switch_model_' + \
        Planter_config['target config']['device'] + \
        '_' + Planter_config['target config']['type']
...
def add_run_switch_model(fname, config):
    ...
    with open(fname, 'w') as command:
        ...
        command.write("cp "+work_root+"/P4/"+file_name+".p4 " +
                      model_test_root + "/"+file_name+".p4\n")
        command.write("sudo p4c --target bmv2 --arch v1model --std p4-16 " + model_test_root +
                      "/"+file_name+".p4\n")  # + ' > '+work_root+'/src/temp/p4c_compile.txt\n')
        # + ' > '+work_root+'/src/temp/start_simple_switch_model.txt\n')
        command.write("sudo simple_switch -i 0@veth0 -i 1@veth1 " +
                      model_test_root + "/"+file_name+".json\n")
...
def add_load_table(fname, config):
    ...
    with open(fname, 'w') as command:
        command.write("#!/bin/bash\n")
        # + ' > ' + work_root + '/src/temp/simple_switch_CLI_load_table.txt\n')
        command.write("simple_switch_CLI <" + work_root +
                      "/Tables/s1-commands.txt ")
    os.chmod(fname, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)


def add_test_model(fname, config):
    test_file_name = 'test_switch_model_' + config['target config']['device'] + '_' + \
        config['target config']['type']
    test_file = config['directory config']['work'] + \
        '/src/test/' + test_file_name + '.py'
    work_root, model_test_root, file_name, test_file_name = file_names(config)

    with open(fname, 'w') as command:
        command.write("#!/bin/bash\n")
        command.write("sudo python3 "+test_file)
    os.chmod(fname, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
...
def main(if_using_subprocess):
    if platform.system() != 'Linux':
        print('Your system is '+platform.system() +
              ' but not linux, please make sure bmv2, p4c and mininet is installed on your os.')
        exit()
    ...
    Planter_config['test config']['port'] = input(
        "- Send packets to which port? (default = 'eth0'): ") or 'eth0'

    json.dump(Planter_config, open(
        'src/configs/Planter_config.json', 'w'), indent=4, cls=NpEncoder)

    ##################################################
    work_root, model_test_root, file_name, test_file_name = file_names(
        Planter_config)
    # =================== compile command for the generated model ===================
    run_switch_model_command = Planter_config['directory config']['work'] + \
        '/src/scripts/run_switch_model.sh'
    add_run_switch_model(run_switch_model_command, Planter_config)

    # =================== load table command for the generated model ===================
    load_table_command = Planter_config['directory config']['work'] + \
        '/src/scripts/simple_switch_CLI_load_table.sh'
    add_load_table(load_table_command, Planter_config)

    # =================== load table command for the generated model ===================
    test_model_command = Planter_config['directory config']['work'] + \
        '/src/scripts/test_switch_model.sh'
    add_test_model(test_model_command, Planter_config)
